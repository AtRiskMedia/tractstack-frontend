...
export const Belief = ({
    ...
    const knownOffset =
      typeof hasMatchingBelief?.verb === `string`
        ? thisScale
            ...
            .at(0)
        : false;
    if (knownOffset && knownOffset?.slug) {
  ...
  return (
    <>
      ...
      {hasSelect ? (
        <div className="mt-6 text-black text-sm font-main font-normal">
          You selected: <span className="font-bold">{selected.name}</span>
          <span className="ml-2">
            <button onClick={() => handleReset()}>
              <XMarkIcon
                className="h-5 w-5 text-mylightgrey hover:text-myblue"
                aria-hidden="true"
              />
            </button>
          </span>
